    ...
    <header class="page-header">
      <div class="left">
        ...
        <a href="https://gitee.com/xuliangzhan_admin/vxe-table/stargazers">
          <img
            src="https://gitee.com/xuliangzhan_admin/vxe-table/badge/star.svg?theme=gvp"
            alt="star"
          />
        </a>
        <a href="https://github.com/x-extends/vxe-table/stargazers">
          <img
            src="https://img.shields.io/github/stars/xuliangzhan/vxe-table.svg"
          />
        </a>
      ...
      <div class="right">
        <div class="content">
          <span
            v-if="appData.usedJSHeapSize && appData.usedJSHeapSize !== '0'"
            class="performance"
          ...
          >
            ...
            <vxe-option
              value="2"
              :label="$t('app.body.other.v2')"
              class-name="due-to-stop"
            ></vxe-option>
            <vxe-option
              value="1"
              :label="$t('app.body.other.v1')"
              class-name="due-to-stop"
            ></vxe-option>
          </vxe-select>
          <router-link
            class="link donation"
            :title="$t('app.footer.donationDesc')"
            :to="{ name: 'Donation' }"
            >{{ $t("app.header.label.donation") }}</router-link
          >
          <template
            v-if="showExtendPlugin && appData.apiLoading && appData.showPlugin"
          >
            <a
              v-if="appData.disabledPlugin"
              class="link support"
              :href="pluginApiUrl"
              target="_blank"
              >💡插件</a
            ...
            <a
              ...
              title="维护中"
              class="link support"
              style="cursor: no-drop; color: #bfbfbf; background-color: #fff"
              @click="$XModal.alert('维护中...', '维护中')"
              >插件</a
            >
          </template>
        </div>
    ...
    <div class="page-container">
        ...
        <div class="header">
          <div class="version-list">
            <template v-if="appData.stableVersionList.length">
              <span class="title">{{
                $t("app.body.label.stableVersion")
              }}</span>
              <vxe-select
                class="stable-select"
                v-model="appData.selectStableVersion"
                size="mini"
                :options="appData.stableVersionList"
              ></vxe-select>
            </template>
            <template v-if="showBetaVetsion">
              <span class="title">{{
                $t("app.body.label.latestVersion")
              }}</span>
              <vxe-select
                class="latest-select"
                v-model="appData.selectBetaVersion"
                size="mini"
                :options="newBetsVersionList"
              ></vxe-select>
            </template>
          </div>
          <vxe-input
            ...
            type="search"
            class="search-input"
            :placeholder="$t('app.body.search.searchPlaceholder')"
            @keyup="searchEvent"
            @clear="searchEvent"
          ></vxe-input>
        </div>
          ...
          <div class="docs">
            <template v-if="appData.apiList.length">
              <ul class="nav-menu">
                ...
                <li
                  v-for="(item, index) in appData.apiList"
                  :key="index"
                  :class="{ expand: item.expand }"
                >
                  <a
                    class="nav-link"
                    @click="linkEvent(item)"
                    :title="
                      item.disabled ? $t('app.body.other.newFunc') : item.label
                    "
                    :class="{
                      disabled: item.disabled,
                      active: pageKey === item.value,
                    }"
                  >
                  ...
                  >
                    <li
                      ...
                      :class="{
                        'is-donation':
                          child.locat &&
                          ['Donation'].includes(child.locat.name),
                      }"
                    >
                      <a
                        class="nav-link disabled"
                        v-if="child.disabled"
                        :title="$t('app.body.other.newFunc')"
                        v-html="child.label"
                      ></a>
                      <a
                        class="nav-link"
            ...
            <template v-else>
              <div class="search-nodata">
                {{ $t("app.body.search.noDataPrefix")
                }}<span class="keyword-lighten">{{ appData.filterName }}</span
                >{{ $t("app.body.search.noDataSuffix") }}
              </div>
            </template>
          </div>
        </div>
      ...
      <div class="oper-wrapper" v-show="showOperBtn">
        <vxe-button
          class="oper-btn"
          :icon="
            appData.showLeft ? 'vxe-icon-arrow-left' : 'vxe-icon-arrow-right'
          "
          @click="appData.showLeft = !appData.showLeft"
        ></vxe-button>
      </div>
      <div class="body">
        <div
          class="content"
          :class="{ full: ['VXEAPI', 'Donation'].includes($route.name) }"
        >
          <div class="page-view">
          ...
          <div class="footer-link">
            <router-link
              v-if="currentLink.preLink"
              class="pre-link"
              :to="currentLink.preLink.locat"
              >{{ currentLink.preLink.label }}</router-link
            >
            <router-link
              v-if="currentLink.nextLink"
              class="next-link"
              :to="currentLink.nextLink.locat"
              >{{ currentLink.nextLink.label }}</router-link
            >
          </div>
          <footer class="page-footer">
            <div>
              <a class="link" href="http://beian.miit.gov.cn/" target="_blank"
                >粤ICP备2022010374号-1</a
              >
            </div>
          </footer>
        </div>
...
export default defineComponent({
  setup () {
    ...
    const getVersion = () => {
      fetch(`${serveApiUrl.value}/api/npm/versions/vxe-table`, {
        method: 'GET'
      })
        ...
        .then((data) => {
          ...
          const stableVersionList = data[`v${appData.version}StableList`].map(
            (value: any) => ({ value, label: value })
          )
          const betaVersionList = data[`v${appData.version}BetaList`].map(
            (value: any) => ({ value, label: value })
          )
          appData.stableVersionList = stableVersionList
          appData.betaVersionList = betaVersionList
          if (stableVersionList.length) {
            appData.selectStableVersion =
              tags && tags[`xtable-v${appData.version}`]
                ? tags[`xtable-v${appData.version}`]
                : stableVersionList[0].value
          }
    ...
    const handleSearch = () => {
      const filterName = XEUtils.toValueString(appData.filterName)
        .trim()
        .toLowerCase()
      if (filterName) {
        ...
        const rest = XEUtils.searchTree(
          appData.tableData,
          (item: any) =>
            item.label.toLowerCase().indexOf(filterName) > -1 ||
            (item.keywords &&
              item.keywords.some(
                (key: string) => key.toLowerCase().indexOf(filterName) > -1
              ))
        )
        XEUtils.eachTree(rest, (item: any) => {
          item.label = item.label.replace(
            filterRE,
            (match: any) => `<span class="keyword-lighten">${match}</span>`
          )
        })
    ...
    // 调用频率间隔 500 毫秒
    const searchEvent = XEUtils.debounce(handleSearch, 500, {
      leading: false,
      trailing: true
    })
    ...
    const showBetaVetsion = computed(() => {
      ...
      if (stableVersionList.length) {
        if (betaVersionList.length) {
          const stableNums = stableVersionList[0].value
            .split('-')[0]
            .split('.')
        ...
        return betaVersionList.some(
          (item: any) => item.value.indexOf('4.') === 0
        )
      }
      return false
    })
    ...
    const currentLink = computed(() => {
      const $route = router.currentRoute.value
      const childArr = XEUtils.toTreeArray(appData.tableData).filter(
        (data) => data.locat && data.locat.name !== 'VXEAPI'
      )
      const cindex = childArr.findIndex(
        (item) => (item.locat && item.locat.name) === ($route && $route.name)
      )
      return {
    ...
    const defaultExpand = () => {
      const group = appData.apiList.find(
        (item) => item.value === pageKey.value
      )
      if (group) {
        group.expand = true
      }
      ...
      if (process.env.NODE_ENV === 'development') {
        setInterval(() => {
          ...
          if (performance && performance.memory) {
            appData.usedJSHeapSize = XEUtils.toFixed(
              XEUtils.floor(performance.memory.usedJSHeapSize / 1048576, 2),
              2
            )
          }
        }, 3000)
